...
for c=1:n_rounds
    ...
    [history_alquier_upper_h(c), history_alquier_lower_h(c)]=alquier_montecarlo_hoeffding(lossy(X,theta_all),p_posterior,p_prior,'gaussian',delta);
    ...
    display(history_upper)
    display(history_lower)
    display(history_alquier_upper)
    display(history_alquier_lower)
end
%plotting
...
%p4h=plot(x,history_alquier_upper_h,color="#77AC30",LineWidth=linewidth);
%p4lh=plot(x,history_alquier_lower_h,color="#77AC30",LineWidth=linewidth);
