...
p_prior=binopdf(l,6,0.7);
p_posterior=binopdf(l,6,0.8);
...
for z=1:repetitions
    ...
    for c=1:n_rounds
        ...
        history_baseline(c)=0.5;
        
        options = optimoptions('fmincon','Algorithm','interior-point','Display','iter');
        [mu_upper, funcval_upper]=fmincon(f_neg,ones(length(theta),1)*3/4,[],[],[],[],zeros(length(theta),1),ones(length(theta),1),gfun,options);
        [mu_lower, funcval_lower]=fmincon(f,ones(length(theta),1)/4,[],[],[],[],zeros(length(theta),1),ones(length(theta),1),gfun,options);
        [nu_upper, funcval_upper_nu]=fmincon(f_neg,ones(length(theta),1)*3/4,[],[],[],[],zeros(length(theta),1),ones(length(theta),1),hfun,options);
        [nu_lower, funcval_lower_nu]=fmincon(f,ones(length(theta),1)/4,[],[],[],[],zeros(length(theta),1),ones(length(theta),1),hfun,options);
